...
> See [`docs/porting.md`](../extending/porting.md) for the porting guide.
...
[`docs/porting.md`](../extending/porting.md). Live widgets additionally need a kernel with real comm
support:
...
| Pluto.jl | no (one-way `PlutoChannel`) | shipped—live-verified demo (`examples/notebooks/pluto_demo.jl`) |

See `docs/using/hosts.md` for the channel abstraction behind this table and
`docs/using/static-export.qmd` for the runnable recipe.
...
[reference → Specifications](../extending/specs.qmd) page from `openspec/specs/`—that page
*is* the spec, not a paraphrase of it.

## Getting started

- [Quickstart: run your first widget](quickstart.qmd) — pick your language
- [Architecture overview](../extending/architecture.qmd) — how cositos works
- [Author your own widget](authoring-widgets.qmd) — write custom ESM
