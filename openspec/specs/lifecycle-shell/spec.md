# Lifecycle Shell Specification

## Purpose
Define the thin imperative shell that calls the lifecycle reducer and executes the returned
effects, bridging between the pure reducer and the real transport/kernel APIs.
...
- **THEN** the shell SHALL call `transport.send(msg_type, data, buffers, metadata)` with the exact parameters from the effect
...
- **THEN** the shell SHALL call `transport.on_message(callback)` where `callback` feeds inbound data back as `inbound` events to `reduce`
...
- **THEN** the shell SHALL call the host's `on_custom(content, buffers)`
...
The shell SHALL query the transport's capability flags once and pass them as a `TransportCapabilities` struct to every `reduce` call. The reducer uses these flags to decide which effects to produce.
...
- **THEN** the returned effects SHALL NOT contain a `listen` effect
...
- **THEN** any returned `send` effects SHALL carry an empty buffers list
...
- **THEN** the returned effects SHALL contain an `error` effect
...
- **WHEN** `_repr_mimebundle_()` is called
- **THEN** the shell SHALL call `reduce` with an `open` event before producing the mimebundle
- **AND** the returned bundle SHALL reference the widget's `model_id`